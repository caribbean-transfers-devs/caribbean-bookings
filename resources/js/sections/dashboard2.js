        ...
        */
        ...
        function sales_day(){
            return optionsBookingsDay = {
                ...
                xaxis: {
                    type: 'datetime',
                    categories: dashboard.seriesBookingsDay().labels,
                    axisBorder: {
                        show: false
                    },
                    axisTicks: {
                ...
                tooltip: {
                    ...
                    custom: function({ series, seriesIndex, dataPointIndex, w }) {
                        console.log( series, seriesIndex, dataPointIndex, w );
                        const details = dashboard.seriesBookingsDay().series[seriesIndex].data[dataPointIndex].details;
                        return `<div class="custom-tooltip">
                                    <div class="apexcharts-tooltip-y-group">
                                        <span class="apexcharts-tooltip-text-y-label">Total de ventas:</span>
                                        <span class="apexcharts-tooltip-text-y-value">${details.counter}</span>
        ...
        function eventData(__status){
            console.log(__status.value);
            const __status_event = document.querySelectorAll('.' + __status.value + '_status');
            if( __status.value == "day" ){
                const __months = document.querySelectorAll('.month_status');
                if( __months.length > 0 ){
                    __months.forEach(__month => {
                        __month.classList.add('d-none');
                    });
                }
            }else{
                const __days = document.querySelectorAll('.day_status');
                if( __days.length > 0 ){
                    __days.forEach(__day => {
                        console.log(__day);                            
                        __day.classList.add('d-none');
                    });
                }
            }

            var options = ( __status.value == "day" ? sales_day() : sales_month() );
            
            if (typeof chart !== 'undefined' && chart !== null) {
                chart.destroy(); // Eliminar la gráfica actual
            }
            var bookingsAnalyticsMonth = new ApexCharts( document.querySelector("#stacked-column-chart"), options );
            bookingsAnalyticsMonth.render();            

            if( __status_event.length > 0 ){
                __status_event.forEach(__statu_event => {
                    __statu_event.classList.remove('d-none');
                });
            }            
        }        
