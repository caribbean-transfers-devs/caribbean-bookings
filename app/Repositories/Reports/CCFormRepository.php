...
class CCFormRepository
{
    ...
    public function createPDF($request){
        ...
        $payments = DB::select("SELECT reservation_id, payment_method, total, currency, object, reference, created_at
                                    FROM payments 
                                WHERE reservation_id IN (" . implode(',', $ids) . ") 
                                    AND payment_method IN('CARD','PAYPAL') 
        ...
        if(sizeof($payments) >= 1):
            foreach($payments as $key => $value):
                if(in_array( $value->payment_method, ['CARD','PAYPAL'] )):
    ...
    public function Stripe($value, $info){
        ...
        $card_type = "";
        if($value->object['payment_method_details']['card']['brand'] == "mastercard"):
            $card_type = "MASTERCARD";
        endif;
        if($value->object['payment_method_details']['card']['brand'] == "visa"):
            $card_type = "VISA";
        endif;
        if($value->object['payment_method_details']['card']['brand'] == "amex"):
            $card_type = "AMEX";
        endif;
