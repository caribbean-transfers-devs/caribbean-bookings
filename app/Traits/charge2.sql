...
FROM reservations as rez
    ...
    LEFT JOIN (
        SELECT 
            ...
            SUM(CASE 
                WHEN payment_method = 'PAYPAL' AND is_conciliated = 1 THEN 
                    CASE 
                        WHEN operation = 'multiplication' THEN total * exchange_rate
                        WHEN operation = 'division' THEN total / exchange_rate
                        ELSE total 
                    END
                ELSE 0 END) AS paypal_conciliated,
            SUM(CASE 
                WHEN payment_method = 'PAYPAL' AND is_conciliated = 0 THEN 
                    CASE 
                        WHEN operation = 'multiplication' THEN total * exchange_rate
                        WHEN operation = 'division' THEN total / exchange_rate
                        ELSE total 
                    END
                ELSE 0 END) AS paypal_not_conciliated,
        ...
        FROM payments
        WHERE payment_method = 'PAYPAL' AND DATE(created_at) = DATE(NOW())
        GROUP BY reservation_id
...
WHERE rez.site_id NOT IN(21,11) AND rez.created_at BETWEEN '2024-12-01 00:00:00' AND '2024-12-31 23:59:59'
